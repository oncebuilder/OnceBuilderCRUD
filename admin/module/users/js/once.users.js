...
once.users = {
	...
	itemEditPreviewRaw: function(obj){
		var id=$("#user-data").data('id');
		window.open(once.path+'/users/'+id+'/user.html','_blank');
	},
	itemNew: function(obj){
		// Call to del_layer with parm id
		$.ajax({
			type: 'POST',
			url: once.path+"/ajax.php?c=users&o=item_new&type_id="+$("#users-data").data('type_id')+"&type_id="+$("#users-data").data('type_id'),
			success: function(data) { 
				// Refresh items list if response ok
				if(data.status=='ok'){
					// Call for refresh
					$.get(once.path+"/view.php?c=users&o="+$("#users-data").data("o")+"&type_id="+$("#users-data").data("type_id")+"&type_id="+$("#users-data").data("type_id"), function(data) {
						$("#content-body").html(data);
					})
	...
	editInit: function(obj){
		...
		$('#user-data a[data-toggle="tab"]').on('show.bs.tab', function (e) {
			...
			if(tab.attr('href')=='#edit_settings'){
				...
				$("#user-data").data("source",false);
				$("#user-data").data("content",false);
				...
				$("#user-data").data("source",false);
				$("#user-data").data("content",true);
			}else if(tab.attr('href')=='#edit_source'){
				// Setting data to make it work with keypress
				$("#user-data").data("source",true);
				$("#user-data").data("content",false);
				...
				// Set other settings
				$("#user-data").data("editor",tab.attr('data-editor'));
				$("#user-data").data("path",tab.attr('data-path'));
				$("#user-data").data("file",tab.attr('data-file'));
				$("#user-data").data("title",tab.html());
		...
		// Save item
		$("#user-data .item-presave").click(function () {
			// item save & source and change location
			$("#editForm").submit();
		});
		
		// Change logo
		$("#user-data .item-logo").click(function () {
			once.users.itemEditLogo($(this));
		});
		// Initialize editForm
...
once.users.dialogs = {
	...
	itemImport: function(obj){
		...
		$(obj).click(function () {
			$("#item-import").load(once.path+"/dialog.php?c=users&o=import", function() {
	...
	itemPublish: function(obj){
		...
		$(obj).click(function () {
			$("#item-publish").load(once.path+"/dialog.php?c=users&o=publish", function() {
	...
	itemReport: function(obj){
		...
		$(obj).click(function () {
			$("#item-report").load(once.path+"/dialog.php?c=users&o=report&id="+$("#user-data").data("id"), function() {
			...
			success: function(data){
				...
				if(data.status=='ok'){
					...
					if($("#user-data").data('redirect')==undefined){
						...
						var type_id=$("#editForm input[name='type_id']");
						...
						if(type_id!=1){
							...
							$("tr[data-id='"+data.item.id+"'] td[data-link='status']").html('<span class="label label-success">Not actived</span>');
						}
					}
	...
	uploadImage: function(obj){
		...
		var options = { 
			...
			success: function(data){
				...
				if(data.status=='ok'){
					$("#item-logo").attr("src","/admin/users/"+data.item.id+"/logo.jpg?"+Math.random());
				}
			},
			complete: function(data){
				//console.log(data.responseText);
